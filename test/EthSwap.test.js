...
contract ('SmartContract EthSwap', ([deployer, investor1, investor2])=>{
    ...
    describe('Token deployment', async ()=>{
        it('Contract has a name', async ()=> {
            ...
            assert.equal(name, 'EXNentum token')
        })        
    })
    ...
    describe('buyTokens() function', async ()=>{
        ...
        it ('Allows users to instantly purchase tokens from EthSwap for a fixed price', async () =>{            
            //check investor EXN balance after purchase
            let investorBalance = await token.balanceOf(investor1);
            assert.equal(investorBalance.toString(),tokens('100'));
            //check ethSwap EXN balance after purchase
            ...
            const event = result.logs[0].EXNs;            
    ...
    describe ('sellTokens() function', async ()=> {
        let result;
        before (async () => {
            // Investor must approve ethSwap to transfer its EXN token (as per ERC20 std)
            await token.approve(ethSwap.address, tokens('100'), {from: investor1});
            result = await ethSwap.sellTokens(tokens('100'), {from: investor1});
        })
        it ('Allow users to instantly swap back EXN tokens for ETHER', async ()=> {
            // Check investor balance in EXN after swap
            let investorBalance = await token.balanceOf(investor1);
            ...
            // Check ethSwap balance in EXN after SWAP
            let ethSwapBalance = await token.balanceOf(ethSwap.address);
